...
Graph::Graph() {
	...
	for (auto& it : L) it = -1;
	for (auto& it : K) it = -1;
	int s_head{ 0 }; // указатель на голову стека
	...
	for (int v(0); v < n; ++v) {
		...
		while (true) {
			...
			if (e != -1) {
				...
				S[s_head] = curr_v; // Заносим вершину в стек
				++s_head; // Размер стека увеличился
				curr_v = next_v; // Текущей вершиной становится следующая вглубь
			}
			else {
				// Делаем шаг назад
				if (s_head == 0) break; // Стек пуст и мы обошли всю компоненту
				else {
					--s_head; // удаляем помеченную и просмотренную вершину из стека
					curr_v = S[s_head]; // переходим к следующей в стеке вершине
				}
			}
		}
	}
